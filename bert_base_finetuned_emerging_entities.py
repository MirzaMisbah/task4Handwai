...
df = pd.read_csv("data/wnut17train.conll", sep="\t", header=None, names=['Word', 'Tag']).fillna(method="ffill")
dev_df = pd.read_csv("data/emerging.dev.conll", sep="\t", header=None, names=['Word', 'Tag']).fillna(method="ffill")
...
sentence_no = 0
sentence = []
for w in df['Word'].values.tolist():
...
sentence_no = 0
sentence = []
for w in dev_df['Word'].values.tolist():
    sentence.append(sentence_no)
    if w == ".":
        sentence_no = sentence_no + 1

dev_df.insert(0, 'Sentence #', sentence)
...
df['Word'] = df['Word'].apply(emoji)
dev_df['Word'] = dev_df['Word'].apply(emoji)
'''
...
for _ in trange(epochs, desc="Epoch"):
    ...
    if f1_scr >= f1_min_score:
        ...
        torch.save(model.state_dict(), 'model/bert_base_emerging_entities_1.pt')
        f1_min_score = f1_scr
